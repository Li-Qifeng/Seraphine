...
> ⚠️ **二次开发声明**
>
> 本项目是 [Zzaphkiel/Seraphine](https://github.com/Zzaphkiel/Seraphine) 的二次开发版本，由 **[Li-Qifeng](https://github.com/Li-Qifeng)** 维护。
> 原项目版权归 Zzaphkiel 所有，遵循 [GPLv3](./LICENSE) 协议；本 fork 的修改同样遵循 GPLv3，禁止商用。
> 问题反馈请前往 [本 fork 的 Issues](https://github.com/Li-Qifeng/Seraphine/issues)，原项目的 Issues 不处理本 fork 的问题。

<div align="center">

  [![License](https://img.shields.io/github/license/Li-Qifeng/Seraphine?style=flat&label=License)](https://github.com/Li-Qifeng/Seraphine/blob/main/LICENSE)
  [![Forks](https://img.shields.io/github/forks/Li-Qifeng/Seraphine?style=flat&label=Forks)](https://github.com/Li-Qifeng/Seraphine/forks)
  [![Stars](https://img.shields.io/github/stars/Li-Qifeng/Seraphine?style=flat&label=Stars)](https://github.com/Li-Qifeng/Seraphine/stargazers)
  [![Downloads](https://img.shields.io/github/downloads/Li-Qifeng/Seraphine/total?style=flat&label=Downloads)](https://github.com/Li-Qifeng/Seraphine/releases)

</div>

## 快速上手 🤗
### 直接使用打包好的程序
点击[这里](https://github.com/Li-Qifeng/Seraphine/releases/latest)进入发布页面，选择以下任意方式：

| 方式 | 文件 | 说明 |
|---|---|---|
| 🆕 安装包（推荐） | `SeraphineSetup-*.exe` | Inno Setup 安装包，自动创建桌面快捷方式，支持增量自更新 |
| 便携版 | `Seraphine.7z` | 解压至文件夹，双击 `Seraphine.exe` 即可 |

> 安装包安装路径为 `%LOCALAPPDATA%\Programs\Seraphine`（per-user，无需管理员权限）。

### 或通过本地构建
下载项目源码 `zip` 压缩包解压至文件夹或通过 `git`
```shell
git clone https://github.com/Li-Qifeng/Seraphine.git
cd Seraphine
```
创建并激活新的 conda 环境
```shell
...
```
### 卸载 Seraphine 😑
- **安装包用户**：在系统设置 → 应用 → 已安装的应用中找到 Seraphine 卸载，或运行 `SeraphineSetup-*.exe` 选择卸载
- **便携版用户**：删除 Seraphine 所在文件夹并删除 `%AppData%/Seraphine` 文件夹即可
...
- 其他辅助功能
  ...
  - 游戏功能 
    ...
    - 观战同大区玩家正在进行的游戏 ✅
    - 锁定游戏内设置 ✅
    - 对局结束后自动点赞（PreEndOfGame 触发，对齐 Riot 时序） ✅
      - 随机点赞（默认） ✅
      - 优先好友 ✅
      - 仅好友 ✅
...
## 帮助我们改进 Seraphine 😘
在您的使用过程中，如果遇到程序的任何 BUG 或不符合预期的行为，欢迎提出 [issue](https://github.com/Li-Qifeng/Seraphine/issues)。发布 issue 时请按照模板填写。**发布新 issue 前请先善用搜索功能，看看之前是否讨论过相关或类似的问题！**

如果您有功能上的添加或修改建议，也非常欢迎提出 issue 进行讨论！[PR](https://github.com/Li-Qifeng/Seraphine/pulls) 也大欢迎！
...
如需构建 Inno Setup 安装包，安装 Inno Setup 6 后执行：
``` shell
ISCC.exe packaging\installer\Seraphine.iss /DMyAppVersion={版本号}
```

> 本项目使用 GitHub Actions CI 自动构建并发布 Release，详见 `.github/workflows/build_seraphine.yaml`。
...
## 套盾环节 🛡️
本程序为在 GitHub 仓库 [Zzaphkiel/Seraphine](https://github.com/Zzaphkiel/Seraphine) 开源的代码的二次开发版本，以及在 [Release](https://github.com/Li-Qifeng/Seraphine/releases) 或官方 QQ 群组中上传的二进制文件。本环节旨在让用户更加全面详尽地了解本程序以及可能风险，以便用户在使用本程序前及过程中做出充分的风险评估和明智的决策。
...
## 点个 Star 支持我们 ⭐
<p align='center'>
  <a href="https://github.com/Li-Qifeng/Seraphine/stargazers">
    <img src="https://api.star-history.com/svg?repos=Li-Qifeng/Seraphine&type=Date">
  </a>
</p>

## 致谢 🥳
- **原项目作者**：[Zzaphkiel](https://github.com/Zzaphkiel) —— 感谢其开源 [Seraphine](https://github.com/Zzaphkiel/Seraphine) 项目，本 fork 在其基础上进行二次开发。
- **当前维护者**：[Li-Qifeng](https://github.com/Li-Qifeng)
...
## 许可证 ⚖️
- 对于非商用行为，Seraphine 使用 [GPLv3](https://github.com/Li-Qifeng/Seraphine/blob/main/LICENSE) 许可证。
