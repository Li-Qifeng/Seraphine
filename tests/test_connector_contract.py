...
- 异常分支按契约抛出 (SummonerNotFound/SummonerGamesNotFound/ReferenceError 等)
...
# ---------------------------------------------------------------------------
# 契约: getCurrentSummoner -> dict (含 summonerId) | raise ReferenceError
# ---------------------------------------------------------------------------
...
class TestGetCurrentSummoner:
    ...
    def test_missing_summonerId_raises_reference_error(self, mock_lcu):
        # 契约: 响应无 summonerId (LCU 未就绪) -> ReferenceError
        # retry 对 ReferenceError 不发信号, 重试耗尽后向上抛
        with _patch_get(_resp(json_data={})):
            with pytest.raises(ReferenceError):
                _run(mock_lcu.getCurrentSummoner())
