"""全队 5 档评级结果缓存 (内存 + SQLite 持久化).

进程内 dict (内存), 按 gameId 存评级结果. 重启后从 SQLite 恢复.
...
def setVerdict(gameId: int,
    ...
    _cache[int(gameId)] = entry
    sqlite_cache.set_verdict(int(gameId), winnerRating, loserRating)
...
def getVerdict(gameId) -> Optional[dict]:
    ...
    if cached is not None:
        return cached
    # 内存未命中则从 SQLite 恢复
    from_db = sqlite_cache.get_verdict(gid)
...
def getTeamRating(gameId, isWinner: bool) -> Optional[list]:
    ...
    """
    verdict = getVerdict(gameId)
    if not verdict:
        return None
    return verdict.get('winnerRating' if isWinner else 'loserRating')
